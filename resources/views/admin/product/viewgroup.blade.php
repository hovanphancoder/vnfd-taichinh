...
<div class="page-content">
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="page-box">
                    <form action="{!!action('Admin\ProductController@editGroup', $id)!!}" method="post" enctype="multipart/form-data">
									  ...
									  @if(config('app.locales'))
										...
										<div class="{{($count++==1)?'active':''}} tab-pane" id="tab_{{$lang}}">
											<div class="form-group">
												<label>Title</label>
												<input type="text" name="language-{{$key}}-name" placeholder="Enter Category Title" value="{!!$viewCategory[$key-1]['name']!!}" required>
											</div>
											
											<div class="form-group">
												<label>Slug</label>
												<input type="text" name="language-{{$key}}-slug" placeholder="Enter Category Slug" value="{!!$viewCategory[$key-1]['slug']!!}">
											</div>
											
											<div class="form-group">
												<label class="control-label" for="message">Description</label>
												<textarea name="language-{{$key}}-description" class="form-control" id="message" placeholder="Description">{!!$viewCategory[$key-1]['description']!!}</textarea>
											</div>
										</div>
										@endforeach
										@endif
                                ...
                                <div class="create-page-right">
									...
									<div class="form-group">
                                        ...
                                        <select class="form-control" id="select" name="parent_id">
                                            ...
                                            <option value="{!!$producttype['id']!!}" <?php echo ($producttype['id'] == $viewCategory1->parent_id) ? "selected" : "" ?> >{!!$producttype['name']!!}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
