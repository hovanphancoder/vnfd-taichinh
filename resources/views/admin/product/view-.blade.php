...
<script>
	...
	tinymce.init({
        ...
        editor_selector: "mceEditor",
        selector: '#specification_1', height: 500,
	...
	tinymce.init({
        ...
        editor_selector: "mceEditor",
        selector: '#specification_2', height: 500,
        plugins: ["advlist autolink link image lists charmap preview hr anchor pagebreak filemanager responsivefilemanager", "code searchreplace wordcount visualblocks visualchars insertdatetime media nonbreaking", "table contextmenu directionality emoticons paste textcolor responsivefilemanager fullscreen"],
        toolbar1: "insertfile undo redo | bold italic underline | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent",
        toolbar2: "fontsizeselect | forecolor backcolor | responsivefilemanager | filemanager | link unlink anchor | image media | fullscreen preview code",
        fontsize_formats: '8pt 10pt 12pt 14pt 18pt 24pt 36pt',
							...
							<ul class="nav nav-tabs">
							 ...
							 <div id="seipkkon_tab_content" class="tab-content">
									  ...
									  <br/> 
										...
										<div class="tab-pane" id="tab_{{$lang}}">
											<div class="form-group">
												...
												<input type="text" name="language-{{$key}}-name" placeholder="Enter Name" value="{!!$viewProduct[$key-1]['name']!!}" required>
											</div>

											<div class="form-group">
												<img class="marrig5" src="{!!url('admin_assets/img/'.$key.'.png')!!}"><label>Slug</label>
												<input type="text" name="language-{{$key}}-slug" placeholder="Enter Product Slug" value="{!!$viewProduct[$key-1]['slug']!!}">
											</div>
											<div class="form-group">
												<img class="marrig5" src="{!!url('admin_assets/img/'.$key.'.png')!!}"><label class="control-label" for="message">Description</label>
												<textarea name="language-{{$key}}-description" class="form-control" id="message" placeholder="Description">{!!$viewProduct[$key-1]['description']!!}</textarea>
											</div>

											<div class="page-editor-box form-group">
												<img class="marrig5" src="{!!url('admin_assets/img/'.$key.'.png')!!}"><label>Content</label>
												<textarea name="language-{{$key}}-content" id="mytextarea_{{$key}}">{!!$viewProduct[$key-1]['content']!!}</textarea>
											</div>
											<div class="page-editor-box form-group">
												<img class="marrig5" src="{!!url('admin_assets/img/'.$key.'.png')!!}"><label>Specification</label>
												<textarea name="language-{{$key}}-specification" id="specification_{{$key}}">{!!$viewProduct[$key-1]['specification']!!}</textarea>
											</div>
										</div>
										@endforeach
										@endif
									...
									<div class="col-md-3">
										<div class="create-page-right">
											<div class="form-group">
												...
												<select class="form-control" id="select_cate" name="id_type">
													...
													@endforeach-->
													 ...
													 @if($producttype['level']<=4)
													<option <?php echo ($producttype['id'] == $viewItem->id_type) ? "selected" : "" ?> data-level="{!!$producttype['level']!!}"  value="{!!$producttype['id']!!}">{!!$producttype['name']!!}</option>
													@endif
													@endforeach
												</select>
											</div>
											...
											<table id="option-value<?php echo $option_row; ?>" class="table table-striped table-bordered table-hover">
											  ...
											  <tbody>
												...
												<tr id="option-value-row<?php echo $option_value_row; ?>">
												  ...
												  <td class="text-left"><button type="button" onclick="$(this).tooltip('destroy');$('#option-value-row<?php echo $option_value_row; ?>').remove();" data-toggle="tooltip" title="remove" class="btn btn-danger"><i class="fa fa-minus-circle"></i></button></td>
												</tr>
								...
								<div id="tab-accessories" class="tab-pane fade">
									<div class="col-md-12 create-page-left">
										  <ul class="nav nav-tabs" role="tablist">
										  <?php $count=1;?>
										  @if(config('app.locales'))
											@foreach(config('app.locales') as $lang)
											  <li class="{{($count++==1)?'active':''}}"><a href="#tabac_{{$lang}}" role="tab" data-toggle="tab">{{trans('general.lang_'.$lang)}}</a></li>
											@endforeach
											@endif
										  </ul>
										  <div class="tab-content">
										  ...
										  @if(config('app.locales'))
											@foreach(config('app.locales') as $key=>$lang)
											<div class="{{($count++==1)?'active':''}} tab-pane" id="tabac_{{$lang}}">
												<div class="form-group">
												<div class="row">
													<div class="col-md-8">
														<img class="marrig5" src="{!!url('admin_assets/img/'.$key.'.png')!!}"><label>Description</label>
														<input type="text" name="language-{{$key}}-description" placeholder="Enter description" value='' required>
													</div>
													<div class="col-md-4">
														<img class="marrig5" src="{!!url('admin_assets/img/'.$key.'.png')!!}"><label>P/N</label>
														<input type="text" name="language-{{$key}}-pn" placeholder="Enter P/N" value=''>
													</div>
												</div>
												</div>
											</div>
										 ...
										 <div id="add_accessories" class="btn btn-success" ><i class="fa fa-check"></i>Add</div>
									</div>
									...
									<div class="row">
										<div class="col-md-12">
											<div class="page-box">
												<div class="table-responsive">
													<table id="accessories-list" class="table table-bordered">
														<thead>
															<tr>
																<th>No</th>
																<th>Description</th>
														...
														<tbody>
															@if($accessories)
															@foreach($accessories as $count => $item)
															<tr>
																<td>{!!$count + 1!!}</td>
																<td>{!!$item['description']!!}</td>
																<td>{!!$item['pn']!!}</td>
																<td class="dropdown order-page">
																	<a href="#mdModelDelete" data-id="{!!$item['id']!!}" class="page-table-info" data-toggle="modal" data-toggle="tooltip" data-placement="center" title="Edit"><i class="fa fa-pencil"></i></a>
																	<a class="btn btn-primary" onclick='Delete({!!$item["id"]!!},{!!$viewItem->id!!})' title="Delete"><i class="fa fa-trash"></i></a>
																</td>
															</tr>
															@endforeach
															@endif
